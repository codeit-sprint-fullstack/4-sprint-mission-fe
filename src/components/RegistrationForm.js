...
function RegistrationForm() {
  ...
  } = useCheckInputValid((value) => value.trim() !== '' && Number.isInteger(Number(value)));
  const {
    inputValue: inputTag,
    isValid: isValidTag,
  ...
  const handleCreateClick = async (e) => {
    if (isBtnActive) {
      ...
      try {
        ...
        result = await createProduct(objData);
        if (result) {
  ...
  return (
    <div className="items-container">
      ...
      >
        ...
        />
        {!isValidDesc && <div className="error-message">10자 이상 100자 이내로 입력해 주세요.</div>}
        ...
        />
        {!isValidPrice && <div className="error-message">1자 이상 숫자로 입력해 주세요.</div>}
